 ...
 * BedrockPacketIds
 *
 * Minecraft Bedrock Edition 1.21.60 (Protokol 748) paket ID'leri.
 * Kaynak: Mojang bedrock-protocol-docs + CloudburstMC/Protocol
 */
object BedrockPacketIds {
    ...
    const val LEVEL_SOUND_EVENT_OLD        = 0x18
    ...
    const val ENTITY_FALL                 = 0x25   // removed in newer protos but ID kept
    ...
    const val LEGACY_TELEMETRY_EVENT      = 0x41
    ...
    const val SET_LOCAL_PLAYER_AS_INIT   = 0x71
    ...
    const val REMOVE_OBJECTIVE_2          = 0x7F
    const val CLIENT_CACHE_STATUS         = 0x81
    const val ON_SCREEN_TEXTURE_ANIMATION = 0x82
    const val MAP_CREATE_LOCKED_COPY      = 0x83
    const val STRUCTURE_TEMPLATE_DATA_REQUEST = 0x84
    const val STRUCTURE_TEMPLATE_DATA_RESPONSE = 0x85
    ...
    const val CLIENT_BOUND_DEBUG_RENDERER = 0xA4
    ...
    const val CLIENT_START_ITEM_COOLDOWN  = 0xB0
    const val SCRIPT_MESSAGE              = 0xB1
    const val CODE_BUILDER_SOURCE         = 0xB2
    const val TICK_INFO                   = 0xB3
    const val LevelGroupPacket            = 0xB4
    const val REQUEST_PERMISSIONS_PACKET  = 0xB5
    const val TOAST_REQUEST               = 0xB6
    const val UPDATE_ABILITIES            = 0xB7
    const val UPDATE_ADVENTURE_SETTINGS   = 0xB8
    const val DEATH_INFO                  = 0xB9
    const val EDITOR_NETWORK             = 0xBA
    const val FEATURE_REGISTRY            = 0xBB
    const val SERVER_STATS                = 0xBC
    const val REQUEST_NETWORK_SETTINGS    = 0xC0
    const val GAME_TEST_REQUEST           = 0xC1
    const val GAME_TEST_RESULTS           = 0xC2
    const val UPDATE_CLIENT_INPUT_LOCKS   = 0xC3
    const val CLIENT_CHEAT_ABILITY_PACKET = 0xC4
    const val CAMERA_PRESETS              = 0xC5
    const val UNLOCKED_RECIPES            = 0xC6
    const val CAMERA_INSTRUCTION          = 0xCA
    const val COMPRESSED_BIOME_DEFINITION = 0xCB
    const val TRIM_DATA                   = 0xCC
    const val OPEN_SIGN                   = 0xCD
    const val AGENT_ANIMATION             = 0xCE
    const val REFRESH_ENTITLEMENTS        = 0xCF
    const val PLAYER_TOGGLE_CRAFTER_SLOT_REQUEST = 0xD0
    const val SET_PLAYER_INVENTORY_OPTIONS = 0xD1
    const val SET_HUD                     = 0xD2
    const val AWARD_ACHIEVEMENT           = 0xD3
    const val CLOSE_FORM                  = 0xD4
    const val SERVER_BOUND_LOADING_SCREEN = 0xD5
    const val JMX_JMX_PACKET              = 0xD6

    // ── Yardımcı: ID'den okunabilir isim ─────────────────────────────────
    private val NAME_MAP: Map<Int, String> by lazy {
        BedrockPacketIds::class.java.declaredFields
    ...
    fun nameOf(id: Int): String = NAME_MAP[id] ?: "UNKNOWN(0x${id.toString(16).uppercase()})"

    /** Modüllerin dinlemesi gereken kritik paket ID'leri */
    val COMBAT_RELEVANT = setOf(
        ENTITY_EVENT,         // hasar olayları
        UPDATE_ATTRIBUTES,    // can / efekt
        ANIMATE,              // swing arm
        INVENTORY_TRANSACTION,// attack
        PLAYER_AUTH_INPUT,    // hareket + aksiyon
        INTERACT,             // interact
        MOB_EQUIPMENT,        // el item değişimi
        ITEM_STACK_REQUEST,   // slot istekleri (totem swap)
        MOVE_PLAYER,
